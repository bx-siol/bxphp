...
class UploadFile {
    ...
    private $config =   array(
        'maxSize'           =>  1024000,    // 上传文件的最大值
        'supportMulti'      =>  true,    // 是否支持多文件上传
        'allowExts'         =>  array('jpg', 'gif', 'png', 'jpeg','mp3','mp4','wav','csv','doc','docx'),    // 允许上传的文件后缀 留空不作后缀检查
        'allowTypes'        =>  array('image/jpg', 'image/gif', 'image/png', 'image/jpeg','image/jpeg','audio/mp3','audio/x-wav','application/vnd.ms-excel','application/octet-stream','text/csv','application/msword','application/vnd.openxmlformats-officedocument.wordprocessingml.document','video/mp4'),    // 允许上传的文件类型 留空不做检查
        'thumb'             =>  true,    // 使用对上传图片进行缩略图处理
        'thumbMaxWidth'     =>  '1920',// 缩略图最大宽度
        'thumbMaxHeight'    =>  '1334',// 缩略图最大高度
        'thumbPrefix'       =>  '',// 缩略图前缀 thumb_
        'thumbSuffix'       =>  '',// 缩略图后缀
        'thumbPath'         =>  '',// 缩略图保存路径
        'thumbFile'         =>  '',// 缩略图文件名
        'thumbExt'          =>  '',// 缩略图扩展名        
        ...
        'savePath'          =>  ROOT_PATH.'uploads/',// 上传文件保存路径
        'autoCheck'         =>  true, // 是否自动检查附件
        'uploadReplace'     =>  false,// 存在同名是否覆盖
        'saveRule'          =>  'uniqid',// 上传文件命名规则
        'hashType'          =>  'md5_file',// 上传文件Hash规则函数名
		'is_fixed'			=>	false, //是否生成固定宽高的缩略图
        );
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
					...
					$this_fixed=$this->is_fixed[$i];
                    Image::thumb($filename,$thumbPath.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true,$this_fixed);                    
                }
