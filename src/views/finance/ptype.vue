<template>
    <Page url="c=Finance&a=ptype" ref="pageRef">
...
const edit = (idx: number, item: any) => {
    ...
    dataForm.max = item.max
    dataForm.min = item.min
...
const test = (idx: number, item: any) => {
    http({
        ...
        _alert({
            ...
            onClose: () => {
                if (res.data.pay_url) {
                    if (item.gametype == 1) location.href = 'https://www.gamedreamer.in/Recharge.html?pay_url=' + res.data.pay_url;
                    else location.href = res.data.pay_url;
                }
            }
        })
    })
