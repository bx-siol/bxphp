    ...
    <div class="divs">
        ...
        <Page :need-time="true" XLSXname="提现管理" url="c=Finance&a=cashlog" ref="pageRef" @success="onPageSuccess">
            ...
            <template #table="myScope">
                ...
                <el-table-column prop="pay_type_bf" :label="isTrans ? 'Channel' : '代付通道'" width="100" fixed>
                    <!-- <template #default="{ row }">
                        <div v-if="row.pay_type == null || row.pay_type == ''">{{row.pay_type_bf}}</div>
                        <div v-else>{{row.pay_type}}</div>
                    </template> -->
                </el-table-column>
