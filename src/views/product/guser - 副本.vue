<template>
    <Page url="c=Product&a=order" ref="pageRef">
        <template #btn="myScope">
            <el-button v-if="power.update" type="success" size="small" icon="el-icon-plus" @click="add">添加赠送用户
            </el-button>
        </template>
        ...
        <template #table>
            ...
            <el-table-column prop="goods_name" label="赠送产品" min-width="300"></el-table-column>
            <el-table-column prop="price" label="产品单价"></el-table-column>
            <el-table-column prop="num" label="赠送数量"></el-table-column>
            <el-table-column prop="days" label="收益天数"></el-table-column>
...
const power = reactive({
    update: checkPower('Product_guser_update'),
    delete: checkPower('Product_guser_delete')
})
...
const save = () => {
    ...
    http({
        ...
        if (!configForm.isEdit) {//添加的重新加载
            ...
            actItem.value.account = res.data.account
            actItem.value.nickname = res.data.nickname
            actItem.value.goods_name = res.data.goods_name
        }
        _alert({
            type: 'success',
            message: res.msg,
...
// document.getElementById('diom').innerText = location.href
</script>