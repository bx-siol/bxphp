        ...
        <el-aside :width="menuWidth" style="overflow-x: hidden;background: #ffffff;">
            <el-scrollbar height="100%" view-style="overflow-x:hidden;">
                <el-affix :offset="0.1" v-if="!isShrink">
                    <div @click="onTopSetClick({path:'/',name:'首页'})" style="background: #ffffff;height: 50px;line-height: 50px;padding: 0 20px;cursor: default;font-weight: bold;border-bottom: 1px solid #f2f2f2;box-sizing:border-box;">
                        <i class="el-icon-s-home"></i>{{store.state.config.name}}
                    </div>
                </el-affix>

                <!--菜单-->
                <el-menu :default-openeds="store.state.user.open_menus" :default-active="active" @select="onMenuSelect" style="border: 0;" router>
                    <el-sub-menu :index="vo.path" v-for="(vo,idx) in store.state.user.menus">
                        ...
                        <el-menu-item-group>
                            ...
                            <el-menu-item :index="voo.path" v-for="(voo,idx2) in vo.sub_node" @click="onMenuClick($event,voo)">{{voo.name}}</el-menu-item>
                        </el-menu-item-group>
                    </el-sub-menu>
                </el-menu>
...
<!--                        <el-button type="warning" icon="el-icon-star-off" size="small" circle></el-button>-->
                        <el-button type="danger" icon="el-icon-delete" size="small" @click="clearCache" circle></el-button>
                        &nbsp;&nbsp
                        <el-dropdown>
                            <div style="cursor: pointer;height: 30px;">
                                <span v-if="!store.state.config.is_mobile">{{store.state.user.nickname}}</span>
                                <el-avatar shape="circle" :size="40" :src="imgFlag(store.state.user.headimgurl)" style="vertical-align: middle;position: relative;top:-2px;"></el-avatar>
                            </div>
                            <template #dropdown>
                                <el-dropdown-menu>
                                    <el-dropdown-item icon="el-icon-warning-outline" @click="onTopSetClick({path:'/sys/profile',name:'基本资料'})">基本资料</el-dropdown-item>
                                    <el-dropdown-item icon="el-icon-lock" @click="onTopSetClick({path:'/sys/safety',name:'安全设置'})">安全设置</el-dropdown-item>
                                    <el-dropdown-item divided icon="el-icon-switch-button" @click="onLogout">退出登录</el-dropdown-item>
                                </el-dropdown-menu>
                            </template>
                        </el-dropdown>
                    </el-col>
                ...
                <div style="position: absolute;top:0px;vertical-align: middle;">
                    ...
                    <a href="/" target="_blank"><img src="../assets/images/web.png" style="height: 20px;vertical-align: middle;"/></a>
                </div>
            </el-header>
...
<style>
    ...
    .el-sub-menu__title i:first-child:before{position: relative;top:-1px;}
    .el-sub-menu__title i:last-child:before{position: relative;}
</style>