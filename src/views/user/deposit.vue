<template>
    <div class="recharge">
        ...
        <div class="recharge_wrap">
            ...
            <div class="payinfo" style="margin-top: 1rem;text-align:center">
                <div>
                    <div class="wdInfoItem" style="margin-right:3vw">
                        <div>₹ {{ dataForm.money * tar / 100 }}</div>
                        <div>{{ t('今日收益') }}</div>
                    </div>
                    <div class="wdInfoItem">
                        <div>₹ {{ tar }}</div>
                        <div>{{ t('团队收入') }}</div>
                    </div>
                    <div class="wdInfoItem" style="margin-right:3vw">
    ...
    <span>₹ {{ wallet.balance }}</span>-->
                Deposit In/Out Amount(₹)
            </div>

            <div>
                <div style="display: flex;align-items: center;">
                    <van-field class="fieldbox" v-model="dataForm.money" :placeholder="t('请输入金额')"
                               style="background-color: #fff; border: 1px solid #80808057;">
                        <template #left-icon>
                            <van-image :src="jb" style="width: 1.4rem;height: 1.4rem;margin-left: 0.5rem;" />
                        </template>
                        <template #button v-if="false">
                            <van-button size="small" type="primary" color="rgb(235 23 0)" style="border-radius:4px;"
                                        @click="onClickAll">ALL</van-button>
                        </template>
                    </van-field>
                </div>
                <van-field class="fieldbox" v-if="false" v-model="password2" :placeholder="t('请输入提现密码')"
                ...
                <div style="text-align: center;">
                    <van-button class="submitBtn" style="color: black; background: white; border: 1px solid red; margin-right: 4vw;" @click="onSubmit1">Deposit Out</van-button>
                    <van-button class="submitBtn" @click="onSubmit2">Deposit In</van-button>
                </div>
            </div>
            ...
            <div class="withdrawalNotes">
                <div class="noticeList">
                    <div class="noticeListItem">
                        <span>You can now deposit any amount with your account balance, which will generate an additional 1% income for you every day. After 0:00 every night,your income and principal will be transferred to your account balance at one time!</span>
                    </div>
                </div>
            </div>
