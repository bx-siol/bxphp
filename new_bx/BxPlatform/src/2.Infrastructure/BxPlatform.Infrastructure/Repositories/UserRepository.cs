...
using BxPlatform.Infrastructure.Sharding;
using SqlSugar;

namespace BxPlatform.Infrastructure.Repositories
{
    /// <summary>
    /// 用户仓储实现
    /// 使用 SqlSugar 原生分表功能
    ...
    public class UserRepository : Repository<SysUser>, IUserRepository
    {
        ...
        public async Task<List<SysUser>> GetSubordinatesAsync(int parentId, int level = 1)
        {
            ...
            var condition = level == 1
                ? (SysUser u) => u.ParentIdLevel1 == parentId
                : (SysUser u) => u.ParentIdLevel2 == parentId;
