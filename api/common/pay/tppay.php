...
function payOrder($fin_paylog, $sub_type = '')
{
	...
	$pdata = [
		...
		// amount	是	integer	订单金额，单位分，例金额为102.33，则填： 10233
		// merchantId	是	integer	商户号，开户后提供，例： 100
		// orderId	是	string	商户订单号，50个字符以内，例：12345678
		// timestamp	是	long	时间戳，精确到毫秒，例：1687510573621
		// notifyUrl	是	string	回调通知地址，例：http://localhost/tppay
		// sign	是	string	签名，md5(amount+merchantId+orderId+timestamp+密钥)进行MD5，32位小写加密。密钥开户后提供.
		'merchantId' => $config['mch_id'],
		'orderId' => $fin_paylog['osn'],
		'amount' =>  $fin_paylog['money'] * 100,
		'timestamp' => $timestamp,
	...
	$result = [];
	try {
		$result = curl_post($config['pay_url'], $pdata, 30);
	} catch (\Throwable $th) {
		return ['code' => -1, 'msg' => ''];
	}
...
function paySign($params, $type = 0)
{
	...
	switch ($type) {
		case 0: //代收
			...
			//(amount+merchantId+orderId+timestamp+密钥)
