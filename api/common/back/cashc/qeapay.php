...
$_ENV['PAY_CONFIG']['qeapay'] = [
	'mch_id' => '400026789',
	'qeapay_key' => 'HGLPBQLAYNUTJVGDHFYXNL7QOGLTHVUA',
	'dpay_url' => 'https://payment.qeapay.com/pay/transfer',
	'dnotify_url' => 'http://'.PAY_BACKURL.'/' . 'api/Notify/qeapay/cash'
];

function qeapayCashOrder($fin_cashlog)
{
	if (strlen($fin_cashlog['receive_account']) < 14) {
		//return ['code'=>-1,'msg'=>'Account is too short'];
	}
	$config = $_ENV['PAY_CONFIG']['qeapay'];
	//$bank=Db::table('cnf_bank')->where("id={$fin_cashlog['receive_bank_id']}")->find();
	$rand_arr = [6, 7, 8, 9];
	$phone = $rand_arr[mt_rand(0, count($rand_arr) - 1)] . mt_rand(1000, 9999) . mt_rand(10000, 99999);
	$time = date("Y-m-d", time());
	$logpathd = LOGS_PATH . "qeapay/cash/{$time}.txt";
	$pdata = [
	...
	$pdata['sign'] = qeapayCashSign($pdata);
	$pdata['sign_type'] = 'MD5';

	//$pdata['sign_type'] = 'MD5'; 
	$url = $config['dpay_url'];
	//file_put_contents($logpathd, $url . "\r\n" . var_export($pdata, true) . "\r\n\r\n", FILE_APPEND);
	$result = qeapayCurlPost($url, $pdata, 30);
	if ($result['code'] != 1) {
		return $result;
	}
...
function qeapayCashSign($params)
{
	$config = $_ENV['PAY_CONFIG']['qeapay'];
	ksort($params);
	$signStr = '';
	...
	$signStr .= 'key=' .  $config['qeapay_key'];
	$time = date("Y-m-d", time());
	$logpath = LOGS_PATH . "qeapay/paySign{$time}.txt";
	//file_put_contents($logpath,  "signstr:\r\n" . $signStr . "\r\n", FILE_APPEND);
	$outstr = (md5($signStr));
	//file_put_contents($logpath,  "signstr:\r\n" . $signStr . "\r\n" . $outstr . "\r\n" . var_export($params, true), FILE_APPEND);
	return $outstr;
}
function qeapayCurlPost($url, $data = [], $timeout = 30)
{
	$curl = curl_init();
