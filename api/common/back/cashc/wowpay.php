...
$_ENV['PAY_CONFIG']['wowpay'] = [
	...
	'dpay_url' => 'https://interface.wowpayglobal.com/pay/transfer',
	'dnotify_url' => 'http://'.PAY_BACKURL.'/' . 'api/Notify/wowpay/cash'
];

function wowpayCashOrder($fin_cashlog)
{
	...
	$fliepath =	LOGS_PATH . 'wowpay/cash/' . date("Y-m-d", time()) . '.txt';
	/*
	...
	*/
	...
	$pdata['sign'] = wowpayCashSign($pdata);
	$pdata['sign_type'] = 'MD5';

	$url = $config['dpay_url'];
	file_put_contents($fliepath, $url . "\r\n" . var_export($pdata, true) . "\r\n\r\n", FILE_APPEND);
	$result = wowpayCurlPost($url, $pdata, 30);
	if ($result['code'] != 1) {
		return $result;
	}
...
function wowpayCurlPost($url, $data = [], $timeout = 30)
{
	$curl = curl_init();
