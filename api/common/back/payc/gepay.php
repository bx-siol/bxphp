...
$_ENV['PAY_CONFIG']['gepay'] = [
	...
	'mch_key' => '3814fc3373294772a55e57c3df19f572',
	'pay_url' => 'https://payment.lexmpay.com/pay/web',
	'query_url' => '',
	'notify_url' => 'http://'.PAY_BACKURL. '/api/Notify/gepay/pay',
...
function payOrder($fin_paylog, $sub_type = '')
{
	...
	*/
	...
	$pdata['sign_type'] = 'MD5';
	file_put_contents(LOGS_PATH . 'payOrderge.txt', $config['pay_url'] . "\r\n" . json_encode($pdata, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . "\r\n\r\n ===============", FILE_APPEND);
	$result = payCurlPost($config['pay_url'], $pdata, 30);

	if ($result['code'] != 1) {
		file_put_contents(LOGS_PATH . 'payOrderge.txt',  $result['code']  . "\r\n\r\n", FILE_APPEND);
		return $result;
	}
...
function paySign($pdata)
{
	...
	file_put_contents(LOGS_PATH . 'payOrderge.txt',  "signstr:\r\n" . $str . "\r\n", FILE_APPEND);
	return (md5($str));
}
...
function payCurlPost($url, $data = [], $timeout = 30)
{
	...
	file_put_contents(LOGS_PATH . 'payOrderge.txt', '=============>' . $response  . "\r\n\r\n", FILE_APPEND);
	if ($curl->error) {
		$arrCurlResult = [
			'code' => -1,
