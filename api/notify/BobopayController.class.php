...
class BobopayController extends BaseController
{
    ...
    public function _pay()
    {
        ...
        writeLog('pdata : ' . json_encode($params, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE), 'bobopay/notify/pay');
        if (!$params)
            $params = $_POST;
    ...
    public function _cash()
    {
        $jsonStr = trim(file_get_contents('php://input'));
        $params = json_decode($jsonStr, true);
        writeLog('pdata : ' . json_encode($params, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE), 'bobopay/notify/cash');
        if (!$params)
            $params = $_POST;
