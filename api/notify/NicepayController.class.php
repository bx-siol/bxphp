...
class NicepayController extends BaseController
{
    ...
    public function _pay()
    {
        ...
        writeLog(json_encode('pdata : ' . $params, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE), 'nicepay/notify/pay');
        if (!$params)
            $params = $_POST;
    ...
    public function _cash()
    {
        $jsonStr = trim(file_get_contents('php://input'));
        $params = json_decode($jsonStr, true);
        writeLog(json_encode('pdata : ' . $params, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE), 'nicepay/notify/cash');
        if (!$params)
            $params = $_POST;
