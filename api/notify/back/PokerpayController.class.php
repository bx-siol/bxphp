...
class PokerpayController extends BaseController
{
	...
	public function _cpay()
	{
		...
		$params['callbackUrl'] = urldecode($params['callbackUrl']);

		$url = 'https://api.pokerpay.in/api/pkv2/repayment/create';
	...
	public function _ccash()
	{
		...
		$url = 'https://api.pokerpay.in/api/pkv2/payment/create';
		$result =  $this->CurlPost($url, $params, 30);
		$json_str = json_encode($result, 256);
		echo $json_str;
	}
	...
	public function _pay()
	{
		...
		$logpathd = LOGS_PATH . 'pokerpay/notify/' . "pay" . $time . ".txt";
	...
	public function _cash()
	{
		...
		$logpathd = LOGS_PATH . 'pokerpay/notify/' . "cash" . $time . ".txt";
		...
		$pdata = [
			'osn' => $params['orderNo'],
			'out_osn' => $params['pkOrderNo'],
			'pay_status' => $params['status'] == 'SUCCESS' ? 9 : 3,
			'pay_msg' => $params['pkOrderNo'],
