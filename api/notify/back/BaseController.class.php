...
class BaseController extends CommonCtl
{
	...
	function payCurlPost($url, $data = [], $timeout = 30)
	{
		...
		$logpath = LOGS_PATH . "hupay/Post{$time}.txt";
	...
	//代收回调统一处理函数
	protected function payAct($pdata = [])
	{
		if (!$pdata['successStr']) {
		...
		$time = date("Y-m-d", time());
		$osn = $pdata['osn'];
		$log_file = LOGS_PATH . 'Notify/payact/' . $time . '.txt';
		Db::startTrans();
		try {
			$order = Db::table('fin_paylog')->where("osn='{$pdata['osn']}'")->lock(true)->find();
			...
			try {
				...
				file_put_contents($log_file, "\r\n{$osn} 订单异常" . var_export($ed, true) . "\r\n\r\n", FILE_APPEND);
			}
		} catch (\Exception $e) {
			file_put_contents($log_file, "\r\n{$osn} 订单异常" . var_export($e, true) . "\r\n\r\n", FILE_APPEND);
			Db::rollback();
			if ($pdata['failStr']) {
				exit($pdata['failStr']);
			} else {
