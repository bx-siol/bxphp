...
class DopayController extends BaseController
{
	...
	public function _cpay()
	{
		...
		$url = 'https://dopay.info/api/order/createorder';
		$result =  $this->CurlPost($url, $params, 30);
		$json_str = json_encode($result, 256);
		$time = date("Y-m-d H:i:s", time());
	    $fliepath =	LOGS_PATH . 'dopay/pay/' . date("Y-m-d", time()) . '.txt';
	...
	public function _ccash()
	{
		...
		$url = 'https://dopay.info/api/payout/create';
		$result =  $this->CurlPost($url, $params, 30);
		$json_str = json_encode($result, 256);
		echo $json_str;
	}
	...
	public function _pay()
	{
		...
		$flpath = LOGS_PATH . 'dopay/notify/pay' . date("Y-m-d", time()) . '.txt';
	...
	public function _cash()
	{
		...
		$log_file = LOGS_PATH . 'dopay/notify/cash' . date("Y-m-d", time()) . '.txt';
